...
We'll now call the API for it to check if the user is logged in or not because when we reload the page the page unlogs the user out. In the **app.js** file, we'll create a new method, but we first need a way of actually storing the user data in a session that doesn't expire. We'll use a library called Flask Login for that, so we'll go to the backend again:
